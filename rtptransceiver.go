...
func (me *RtpTransceiver) Receiver() *RtpReceiver {
	...
	if receiver.fd == nil {
		logger.Errorf("Failed to RtpTransceiverGetReceiver.")
		return nil
	}
	return receiver
}
...
func (me *RtpTransceiver) Sender() *RtpSender {
	...
	if sender.fd == nil {
		logger.Errorf("Failed to RtpTransceiverGetSender.")
		return nil
	}
	return sender
}
...
func (me *RtpTransceiver) SetDirection(new_direction string) error {
	...
	if eno != 0 {
		logger.Errorf("Failed to RtpTransceiverSetDirection: name=%s, message=%s", C.GoString(err.name), C.GoString(err.message))
